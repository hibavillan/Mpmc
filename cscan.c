...
// Function to calculate the seek time using C-SCAN algorithm
void cscanDiskScheduling(int requests[], int n, int head, int disk_size, int direction) {
    ...
    if (direction == 1) {
        ...
        // Return to the beginning of the disk and continue servicing requests
        if (head <= disk_size - 1) {
        ...
        // Return to the end of the disk and continue servicing requests
        if (head >= 0) {
            seek_sequence[index++] = disk_size - 1;
        }
        for (int i = n - 1; i >= 0; i--) {
